...
import { AgentNavLinksHome } from "./AgentNavLinksHome";
import { UserCircleAlt, MoreVert, LogOut, Search, Bell } from 'iconoir-react';
import { menuAppBarHomeStyles } from "./menuAppBarHomeStyles";
...
const MenuAppBarHome = () => {
    const classes = menuAppBarHomeStyles();
                    ...
                    <div className={classes.sectionDesktop} style={ ((process.env.REACT_APP_FORCE_LICENSE_CHECK ===  'FALSE') || (profileUser && profileUser.hasValidLicense)) ? { flexGrow: '1'} : undefined}>
                        ...
                        <div style={{alignItems: 'center', display: 'flex'}}>
                            <IconButton aria-label="notification" color="inherit" onClick={gotoMyRequests}>
                                <Badge badgeContent={notifications} color="error" invisible={!notifications}>
                                    <Bell
                                        width={'20px'}
                                        height={'20px'}
                            ...
                            >
                                <span
                                    style={{ marginRight: '20px', color: '#FFF', fontFamily: 'Montserrat', fontWeight: '700' }}
...
export default React.memo(MenuAppBarHome);
